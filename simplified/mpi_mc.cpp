  ...
  if (sz > 1) {
    ...
    for(int i=1;i<=nsweeps;++i)
    {
      ...
      if (uni(rd_sample) <= .33)
      {
        ...
        float E1 = local_energy(lattice, N, i, j, k) + local_energy(lattice, N, x,y,z);
        exchange(lattice, N, red, blue);
        float E2 = local_energy(lattice, N, i, j, k) + local_energy(lattice, N, x,y,z);
...
//sample(const int* __restrict a, const int& N, const int& count, const std::mt19937 &sampler, const int &part_type)
      sweep(lattice, N, beta);
      //tf += MPI_Wtime();
      //if (i % curr == 0)
      if (i == curr)
