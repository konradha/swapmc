  ...
  for (int i = 0; i < N; ++i)
    for (int j = 0; j < N; ++j)
      for (int k = 0; k < N; ++k) {
        ...
        log += (lattice1[idx] == lattice2[idx]) && (lattice1[idx] > 0);
      }
  return log;
}
...
void random_sweep(short *__restrict lattice, const int L,
           std::vector<std::mt19937> &gens,
           std::vector<std::uniform_int_distribution<>> &nn_dis,
           std::vector<std::uniform_real_distribution<>> &unis,
           const float beta,
    ...
    {
        const int site = std::rand() % (L * L * L);
        const auto [i, j, k] = revert(site, L);
        const int mv = nn_dis[0](gens[0]);
        const int nb = lattice[mv];
        const int site_ty = lattice[site];
        if (nb == site_ty) continue;
        const auto [mi, mj, mk] = revert(mv, L);

        const volatile float E1 =
            nn_energy(lattice, L, i, j, k) + nn_energy(lattice, L, mi, mj, mk);
        exchange(lattice, L, site, mv);
        const volatile float E2 =
            nn_energy(lattice, L, i, j, k) + nn_energy(lattice, L, mi, mj, mk);
        const auto dE = (E2 - E1);
        const auto crit = (std::exp(-beta * dE) < 1. ? std::exp(-beta * dE) : 1.);
        if (dE <= 0 || unis[0](gens[0]) < crit) {
          continue;
        } else {
          exchange(lattice, L, site, mv);
        }
...
#pragma omp critical
  ...
  for (int i = 1; i <= nsweeps; ++i) {
    random_sweep(lattice, N, thread_generators, nn_dis, unis, beta, slice_coords);
    if (i == curr)
    {
